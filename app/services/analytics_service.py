...
def _to_utc_naive(dt: datetime) -> datetime:
    """Convert aware datetime to naive UTC for SQL Server DATETIME/DATETIME2 filtering."""
...
def build_analytics_response_for_db(db: Session, *, now_ist: datetime | None = None) -> dict:
    ...
    today_start_utc = _to_utc_naive(today_start)
    tomorrow_start_utc = _to_utc_naive(tomorrow_start)
    ...
    month_start_utc = _to_utc_naive(month_start)
    next_month_start_utc = _to_utc_naive(next_month_start)

    week_start = today_start - timedelta(days=6)
    week_start_utc = _to_utc_naive(week_start)

    total_leads = int(db.query(func.count(Lead.id)).scalar() or 0)
    today_leads = int(
        db.query(func.count(Lead.id))
        .filter(Lead.created_at >= today_start_utc, Lead.created_at < tomorrow_start_utc)
        .scalar()
        or 0
    )
    week_leads = int(
        db.query(func.count(Lead.id))
        .filter(Lead.created_at >= week_start_utc, Lead.created_at < tomorrow_start_utc)
        .scalar()
        or 0
    )
    month_leads = int(
        db.query(func.count(Lead.id))
        .filter(Lead.created_at >= month_start_utc, Lead.created_at < next_month_start_utc)
        .scalar()
        or 0
    )
    chatbot_leads = int(
    ...
    while cursor_ist <= today:
        ...
        day_start_utc = _to_utc_naive(day_start_ist)
        day_end_utc = _to_utc_naive(day_end_ist)
        cnt = int(
            db.query(func.count(Lead.id))
